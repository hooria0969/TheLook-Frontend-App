...
import BidForm from "../my-components/BidForm";
import ListOfBids from "../my-components/ListOfBids";

// import '../TestApp/MyPaintingDetail.css';
...
function PaintingDetails() {
  ...
  useEffect(() => {
    const fetchPaintingDetail = async () => {
      try {
        const response = await axios.get(`https://the-look-backend-app2.vercel.app/PaintingDetails/${id}`);
  ...
  if (loading) return <p>Loading Painting Details...</p>;
  ...
  return (
      ...
      <div className="painting-details-page">
            <div className="painting-section">
                <img src={paintingDetail.paintingImage} alt={paintingDetail.paintingName} />
                <div className="painting-info">
                    <h1>{paintingDetail.paintingName}</h1>
                    <h4>{paintingDetail.artistName }</h4>
                    <p>Minimum Bid Price: ${paintingDetail.price}</p>
                </div>
            </div>

              
            <ListOfBids bids={paintingDetail.bids} />

            {/* <BidForm paintingId={paintingDetail.paintingid} bidId={paintingDetail.bidid} /> */}

            <BidForm bidObj= {{bidid : 5, paingtingid : paintingDetail._id } } />
        </div> 
